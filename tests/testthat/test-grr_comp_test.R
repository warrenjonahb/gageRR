test_that("ANOVA/XBAR_R GRR Computation Check", {
  ...
  expect_equal(round(grr_xbar_check$VarianceComponents["repeatability", "PercentContribution"],3), .357)
  expect_equal(round(grr_xbar_check$VarianceComponents["reproducibility", "PercentContribution"],3), 0)
  expect_equal(round(grr_xbar_check$VarianceComponents["total_grr", "PercentContribution"],3), .357)
  expect_equal(round(grr_xbar_check$VarianceComponents["part_to_part", "PercentContribution"],3), .643)
  ...
  expect_equal(round(grr_xbar_check$GageEval["total_grr", "PercentTolerance"],3), .163)
  expect_equal(round(grr_xbar_check$GageEval["part_to_part", "PercentTolerance"],3), .219)
  expect_equal(round(grr_xbar_check$GageEval["total_var", "PercentTolerance"],3), .273)
