...
The gageRR package provides two methods to analyze repeatability and reproducibility: Analysis of Variance (ANOVA) method and Average and Range method.  These methods require a balanced study, that is the same number of repititions and for each operator and part combination. 

This vignette explains the use of the package and demonstrates typical workflows.

## Reference

Three references were used for the developement of this package:

1. "MSA Reference Manual", 4th Edition, Chrysler Group LLC, Ford Motor Company, General Motors Corporation.

2. "Methods and formulas for gage R&R table in Crossed Gage R&R Study", https://support.minitab.com/en-us/minitab/21/help-and-how-to/quality-and-process-improvement/measurement-system-analysis/how-to/gage-study/crossed-gage-r-r-study/methods-and-formulas/gage-r-r-table/

3. "Six Sigma Study Guide", https://sixsigmastudyguide.com/repeatability-and-reproducibility-rr/
...
The Average and Range Method allows for the calculation of repeatability and reproducibility but will does not account for the interaction between the part and operator. Each variance compnent (VARCOMP) can be defined as follows:
...
$$SS_{Total} = \sum_{i=1}^t \sum_{j=1}^p \sum_{m=1}^r(X_{ijm} - \overline{X}_{ij})^2$$
...
The final sum of squares (Operator * Part) is calcuated as:

$$SS_{Operator * Part} = SS
_{Total} - (SS_{Operator} + SS_{Part} + SS_{Equipment})$$

With the gageRR package we can caclulate the sum of squares with the following command:
