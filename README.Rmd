...
The gageRR package provides two methods to analyze repeatability and reproducibility: Analysis of Variance (ANOVA) method and Average and Range method.  The ANOVA method now fits a mixed-effects model so it can accommodate studies where operators measure parts an unequal number of times (each operator/part pairing still needs at least two measurements).  The Average and Range method requires a balanced study with the same number of repetitions for each operator and part combination.
